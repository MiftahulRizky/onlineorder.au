...
Partial Class Methods_SoftShadeMethod
    ...
    Public Shared Function BindListData(ByVal data As ParamListData) As Object
        Try
            ...
            Select Case data.field.ToLower()
                ...
                Case "controltype"
                    query = String.Format("SELECT Id, ControlType FROM HardwareKits WHERE DesignId='{0}' AND BlindId = '{1}' AND TubeType='{2}' AND Active=1 ORDER BY Name ASC", data.designid, UCase(data.blindtype).ToString(), data.tubetype)
                    resultList = GetFormattedData(query, "Id", "ControlType")
                ...
                Case "fabriclength"
                    data.designid = "B556E35C-CEAC-40F8-A6CF-156601BD57DA"
                    Dim Width As String = ""
                    Select Case data.tubetype
                        Case "Louvolite", "Javaline"
                            Width = "AND Width IN ('89', '127')"
                    End Select
                    query = String.Format("SELECT Width FROM Fabrics WHERE DesignId='{0}' AND Type='{1}' {2} AND Active='1' GROUP BY Width ORDER BY Width ASC", data.designid, data.fabrictype, Width)
                    resultList = GetFormattedData(query, "Width", "Width")

                Case "fabriccolour"
                    data.designid = "B556E35C-CEAC-40F8-A6CF-156601BD57DA"
                    query = String.Format("SELECT Id, Colour FROM Fabrics WHERE DesignId='{0}' AND Type='{1}' AND Width='{2}' AND Active='1'  ORDER BY Name ASC", data.designid, data.fabrictype, data.fabriclength)
        ...
        Try
            ...
            If BlindName = "Slat Only" Then
                If Not String.IsNullOrEmpty(data.slatqty) Then
                    If Not IsNumeric(data.slatqty) Then
                        Return New With { .warning = true, .message = "slat qty is required !", .field = "slatqty"}
                    End If
                End If
            End If
            ...
            If Not BlindName = "Track Only" Then
                ...
                If String.IsNullOrEmpty(data.fabriclength) Then
                    Return New With { .warning = true, .message = "fabric / slat is required !", .field = "fabriclength"}
                End If
            ...
            If BlindName = "Complete" Or BlindName = "Track Only" Then
                If String.IsNullOrEmpty(data.trackcolour) Then
                    Return New With { .warning = true, .message = "track colour is required !", .field = "trackcolour"}
                End If
                If String.IsNullOrEmpty(data.stackposition) Then
                    Return New With { .warning = true, .message = "stack position is required !", .field = "stackposition"}
                End If
            ...
            If ControlType = "Wand" Then
                If String.IsNullOrEmpty(data.wandlength) Then
                    Return New With { .warning = true, .message = "wand length is required !", .field = "wandlength"}
                End If
                If String.IsNullOrEmpty(data.wandcolour) Then
                    Return New With { .warning = true, .message = "wand colour is required !", .field = "wandcolour"}
                End If
                If data.wandlength = "custom" Then
                    If String.IsNullOrEmpty(data.wandcustomlength) Then
                        Return New With { .warning = true, .message = "custom wand length is required !", .field = "wandcustomlength"}
                    End If
                End If
            End If
            ...
            If BlindName = "Complete" Or BlindName = "Track Only" Then
                If String.IsNullOrEmpty(data.bracket) Then
                    Return New With { .warning = true, .message = "bracket is required !", .field = "bracket"}
                End If
                If String.IsNullOrEmpty(data.bracketcolour) Then
                    Return New With { .warning = true, .message = "bracket colour is required !", .field = "bracketcolour"}
                End If
            End If
            
            If String.IsNullOrEmpty(data.hangertype) Then
                Return New With { .warning = true, .message = "hanger type is required !", .field = "hangertype"}
            End If

            If Not BlindName = "Track Only" Then
                If String.IsNullOrEmpty(data.bottom) Then
                    Return New With { .warning = true, .message = "bottom is required !", .field = "bottom"}
                End If
            End If
    ...
    Public Shared Function BindItemOrder(ByVal data As ParamBindItemOrder) As Object
        Try
            ...
            Dim FabricLength As Object  = BindListData(New ParamListData With {
                .field = "fabriclength",
                .designid = data.designid,
                .tubetype = DetailData("TubeType").ToString(),
                .fabrictype = DetailData("FabricType").ToString()
            })
            If FabricLength.error Then
                Throw New Exception(FabricLength.message)
            End If

            Dim FabricColour As Object  = BindListData(New ParamListData With {
                .field = "fabriccolour",
                .designid = data.designid,
                .fabrictype = DetailData("FabricType").ToString(),
                .fabriclength = DetailData("SlatSize").ToString()
            })
            If FabricColour.error Then
                Throw New Exception(FabricColour.message)
            End If
            ...
            Return New With {
                ...
                .FabricLength = FabricLength.list,
                .FabricColour = FabricColour.list
            }
        Catch ex As Exception
