...
Partial Class Methods_Setting_Kit_KitMethod
    ...
    Public Shared Function BindDesignType() As Object
        Try
            Dim datas As DataSet = publicCfg.GetListData("SELECT * FROM Designs WHERE Type IN ('Additional', 'Surcharge', 'Blinds', 'Door', 'Window', 'Door and Window') ORDER BY Name ASC")
    ...
    Public Shared Function BindBlindType(ByVal designid As String) As Object
        Try
            Dim datas As DataSet = publicCfg.GetListData("SELECT * FROM Blinds WHERE DesignId='" + UCase(designid).ToString() + "' AND Company='SP' ORDER BY Name ASC")
            Dim list As New List(Of Dictionary(Of String, String))()
            If datas IsNot Nothing AndAlso datas.Tables.Count > 0 Then
                For Each row As DataRow In datas.Tables(0).Rows
                    Dim result As New Dictionary(Of String, String) From {
