...
Partial Class Setting_Kit_Detail
    Inherits Page
    ...
    Protected Sub Page_Load(sender As Object, e As EventArgs) Handles Me.Load
        ...
        If Session("kitDetail") = "" Then
            Response.Redirect("~/setting/kit", False)
            Exit Sub
        End If

        lblId.Text = UCase(Session("kitDetail")).ToString()
        If Not IsPostBack Then
            Call BackColor()
            Call BindDataKit(lblId.Text)
        End If
    End Sub
    ...
    Protected Sub btnSubmit_Click(sender As Object, e As EventArgs)
        ...
        Try
            ...
            If ddlBlind.SelectedValue = "" Then
                ...
                ddlBlind.CssClass ="form-control  is-invalid"
                ddlBlind.Focus()
                Exit Sub
            End If
            ...
            If msgError.InnerText = "" Then
                ...
                publicCfg.InsertActivity(userId, Page.Title, "UPDATE NEW HARDWARE KIT. ID : " & txtId.Text)
    ...
    Private Sub BindDataKit(Id As String)
        Call BackColor()
        Try
            Dim myData As DataSet = publicCfg.GetListData("SELECT * FROM HardwareKits WHERE Id = '" + Id + "'")
            ...
            Call BindDataDesign()
            Call BindDataBlind(designId)
            Call BindBracketType()
            Call BindTubeType()
            Call BindControlType()
            Call BindColourType()

            txtId.Text = myData.Tables(0).Rows(0).Item("SoeId").ToString()
            ...
            ddlBracketType.SelectedValue = myData.Tables(0).Rows(0).Item("BracketType").ToString()
            ddlTubeType.SelectedValue = myData.Tables(0).Rows(0).Item("TubeType").ToString()
            ddlControlType.SelectedValue = myData.Tables(0).Rows(0).Item("ControlType").ToString()
            ddlColourType.SelectedValue = myData.Tables(0).Rows(0).Item("ColourType").ToString()
    ...
    Private Sub BindDataBlind(DesignId As String)
        ...
        Try
            If Not DesignId = "" Then
                ddlBlind.DataSource = publicCfg.GetListData("SELECT *, UPPER(Name) AS NameText FROM Blinds WHERE DesignId='" + UCase(DesignId).ToString() + "' AND Active=1 ORDER BY Name ASC")
