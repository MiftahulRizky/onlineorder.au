document.addEventListener("DOMContentLoaded", () => {
  if (ROLENAME === "Administrator" || ROLENAME === "PPIC & DE") {
    console.log("GlobalRomanBlinds.js loaded successfully");
...
document.querySelectorAll(".form-control, .form-select").forEach((el) => {
  el.addEventListener("change", async (e) => {
    ...
    if (e.target.id === "materialchain") {
      const materialchain = e.target.value;
      await bindChainColour(materialchain);
    }
  });
...
const bindChainColour = (materialchain) => {
  if (!materialchain) return;
  let list = [];
  if (materialchain == "Chrome") {
    list.push("Chrome");
  }

  if (materialchain == "Plastic") {
    list.push("Ivory", "Black", "White", "Beige", "Grey");
  }

  if (materialchain == "Stainless Steel") {
    list.push("Stainless Steel");
  }
  generateOption("chaincolour", list, 1);
};
  ...
  try {
    ...
    for (const item of data) {
      ...
      await Promise.all([
        ...
        bindControlPosition(),
        bindMaterialChain(),
        bindChainColour(item.MaterialChain),
        bindCordColour(),
...
// ----------------------------------------------|| Other Functions ||---------------------------------------
const romanGlobalPageLoaded = async () => {
  if (!HEADERID) {
