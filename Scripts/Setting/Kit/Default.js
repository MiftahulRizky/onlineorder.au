...
document
  ...
  .addEventListener("change", (e) => {
    ...
    setState("filter_kit_designid", designid);
...
document
  ...
  .addEventListener("change", (e) => {
    ...
    setState("filter_kit_blindid", blindid);
    bindKits(designid, blindid, "#card-table #data-table");
  });

// click button add
document
  .querySelector("#card-table #btn-add")
  .addEventListener("click", async () => {
    ...
    await Promise.all([
      ...
      handlerSelBracket("#modalSubmit #bracket"),
      handlerSelTube("#modalSubmit #tube"),
      handlerSelControl("#modalSubmit #control"),
      handlerSelColour("#modalSubmit #colour"),
    ]);
...
document
  ...
  .addEventListener("click", (e) => {
    if (e.target.matches("#btn-edit")) {
      ...
      const kitid = e.target.getAttribute("data-id");
      handlerEdit(kitid);
    }
  });
...
const handlerSelDesigns = async (params) => {
  ...
  try {
    ...
    if (params === "#card-table #designid") {
      ...
      const currentDesigns = getState("filter_kit_designid") || designid;
  ...
  try {
    ...
    if (Array.isArray(data)) {
      ...
      if (params === "#card-table #blindid") {
        ...
        const currentBlinds = getState("filter_kit_blindid") || blindid;
        sel.value = currentBlinds;
        bindKits(designid, currentBlinds, "#card-table #data-table");
      }
    }
  } catch (error) {
    const msg =
...
const handlerSelBracket = async (params) => {
  const sel = document.querySelector(params);
  sel.innerHTML = ""; // reset

  try {
    const response = await fetch(uriMethod + "/BindBracket", {
      method: "POST",
      headers: {
        "Content-Type": "application/json; charset=utf-8",
      },
...
const handlerSelTube = async (params) => {
  const sel = document.querySelector(params);
  sel.innerHTML = ""; // reset

  try {
    const response = await fetch(uriMethod + "/BindTube", {
      method: "POST",
      headers: {
        "Content-Type": "application/json; charset=utf-8",
      },
...
const handlerSelControl = async (params) => {
  const sel = document.querySelector(params);
  sel.innerHTML = ""; // reset

  try {
    const response = await fetch(uriMethod + "/BindControl", {
      method: "POST",
      headers: {
        "Content-Type": "application/json; charset=utf-8",
      },
...
const handlerSelColour = async (params) => {
  const sel = document.querySelector(params);
  sel.innerHTML = ""; // reset

  try {
    const response = await fetch(uriMethod + "/BindColour", {
      method: "POST",
      headers: {
        "Content-Type": "application/json; charset=utf-8",
      },
...
const handlerEdit = async (kitid) => {
  try {
    ...
    for (const item of data) {
      ...
      await handlerSelBracket("#modalSubmit #bracket");
      await handlerSelTube("#modalSubmit #tube");
      await handlerSelControl("#modalSubmit #control");
      await handlerSelColour("#modalSubmit #colour");
...
// --------------------------------------------------|| Binding Functions ||---------------------------------------
let tableData;
const bindKits = (designid, blindid, params) => {
  if (tableData) {
    tableData.destroy();
  }
